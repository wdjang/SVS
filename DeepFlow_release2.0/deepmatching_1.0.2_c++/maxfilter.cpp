...
*/
static inline float _local_argmax( int l, int u, int v, const float_layers* map, int* x, int* y ) {
...
static float** get_list_corres( float_cube* map, int* nb ) {
  const int tz = map->tz;
  float* m = map->pixels;
...
*/
float* _intersect_corres( float_cube* map0, float_cube* map1, int* nres ) {
  const int tz = 6;
...
static inline float mydot( const float* m, float x, float y ) {
  return x*m[0] + y*m[1] + m[2];
}


static void merge_one_side( float_image* rot, int step, float_cube* corres, float tol,  
                            int all_step, float_cube* all_corres, int offset ) {
  assert( corres->tz==6 && all_corres->tz==6 );
  assert( rot->tx==3 && rot->ty>=2 );
  const float* aff = rot->pixels;
  ...
  for(j=0; j<all_corres->ty; j++) 
    for(i=0; i<all_corres->tx; i++) {
      ...
      float xr = mydot( aff + 0, x, y ); 
      float yr = mydot( aff + 3, x, y );
...
*/
void merge_corres( float_image* rot0, float_image* rot1, int step0, int step1, 
