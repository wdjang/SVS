...
void compute_one_level(image_t *wx, image_t *wy, color_image_t *im1, color_image_t *im2, image_t *desc_flow_x, image_t *desc_flow_y, image_t *desc_weight, const optical_flow_params_t *params){ 
    ...
    image_t *du = image_new(width,height), *dv = image_new(width,height), // the flow increment
        ...
        *smooth_weight = compute_smoothness_weight(im1, 5.0f, deriv); // local weights for smoothness
    ...
    for(i_inner_iteration = 0 ; i_inner_iteration < params->n_inner_iteration ; i_inner_iteration++){
        ...
        compute_smoothness(smooth_horiz, smooth_vert, uu, vv, smooth_weight, deriv_flow, half_alpha );
...
void optical_flow(image_t *wx, image_t *wy, const color_image_t *im1, const color_image_t *im2, optical_flow_params_t *params, const image_t *match_x, const image_t *match_y, image_t *match_z){
    ...
    if(params->beta){
        if(match_x == NULL){
            printf("No descriptor input, weight of descriptor term set to 0\n");    
            params->beta = 0.0f;
            half_beta = 0.0f;
        }else{
            compute_desc_weight(im1, im2, match_x, match_y, match_z, deriv_flow, deriv);
